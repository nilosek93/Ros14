...
bool Mailbox::sendItem(Item* item) const {
	...
	if (player) {
		if (g_game.internalMoveItem(item->getParent(), player->getInbox().get(), INDEX_WHEREEVER,
		...
		if (g_game.internalMoveItem(item->getParent(), tmpPlayer.getInbox().get(), INDEX_WHEREEVER,
		                            item, item->getItemCount(), nullptr, FLAG_NOLIMIT) == RETURNVALUE_NOERROR) {
			g_game.transformItem(item, item->getID() + 1);
